...
const POST = {
  ...
  links: [
    ...
    { label: "Demo", url: "https://www.youtube.com/watch?v=9MJs_bc2I6c", icon: "play", },
    {
      label: "API Docs",
      url: "https://console.bodhan.ai/api-docs/#document-ocr-api",
      icon: "docs",
    },
    {
      label: "Try Out",
      url: "https://console.bodhan.ai/ui/login",
      icon: "bolt",
    },
  ],
};
...
// ── Main page ────────────────────────────────────────────────────────────────
export default function BodhanOCRBlogPage() {
  ...
  const injectArticleStyles = useCallback((doc: Document) => {
    if (!doc.getElementById("ai4b-article-injected")) {
      const style = doc.createElement('style');
      style.id = "ai4b-article-injected";
      style.innerHTML = `
        html, body { background: transparent !important; height: auto !important; min-height: 0 !important; }
        #splash { display: none !important; }
        body { padding-bottom: 3rem !important; }
        .hero { display: none !important; }
        .progress { display: none !important; }
        .rail { display: none !important; }
        footer, .ecosystem { display: none !important; }
        .research-type-dek { margin-top: 0 !important; }
        .column { margin: 0 auto !important; max-width: 800px !important; }
      `;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (iframe?.contentDocument) {
        ...
        // Fallback DOM section extraction if sections is not set by postMessage
        if (sections.length === 0) {
          const headings = Array.from(
            iframe.contentDocument.querySelectorAll("h2, h3, section[id]")
          );
      ...
      const coverIframe = coverIframeRef.current;
      if (coverIframe?.contentDocument) {
        handleCoverLoad();
      }
    }, 500);
    return () => clearInterval(interval);
  }, [handleCoverLoad, injectArticleStyles, sections.length]);
  ...
  // Keep active sidebar link scrolled into view inside sidebar container
  useEffect(() => {
    if (!activeSection || !sidebarRef.current) return;
    try {
      const activeBtn = sidebarRef.current.querySelector(`[data-sec-id="${activeSection}"]`);
      if (activeBtn) {
        activeBtn.scrollIntoView({ block: "nearest", behavior: "smooth" });
      }
    } catch (e) {
      // ignore query selector errors for arbitrary IDs
    }
  }, [activeSection]);
        ...
        <Container maxW="container.lg">
          <Center>
            ...
            >
              <iframe
                ref={coverIframeRef}
                src="/static-blogs/bodhan-ocr/index.html#cover"
        ...
        <Flex justify="center" w="100%" px={{ base: 2, md: 4 }}>
          ...
          >
            ...
            <iframe
              ...
              src="/static-blogs/bodhan-ocr/index.html"
