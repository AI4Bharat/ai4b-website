...
const POST = {
  ...
  links: [
    {
      label: "Demo",
      url: "https://www.youtube.com/watch?v=-qunAAHFvBc",
      embed: "https://www.youtube.com/embed/-qunAAHFvBc?rel=0",
      icon: "play",
    },
    {
      label: "Hugging Face",
...
export default function BodhanASRBlogPage() {
  ...
  // Auto-resize article iframe height and extract sections using polling (immune to cache/postMessage bugs)
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (iframe?.contentDocument) {
        ...
        setSections(prev => {
          ...
          }).filter(s => s.title && s.title !== s.id && s.title !== 'motivation');
    ...
    return () => clearInterval(interval);
  }, [injectArticleStyles, handleCoverLoad]);
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const extractedSections = Array.from(doc.querySelectorAll('section[id]')).map((s: any) => {
        const titleAttr = s.getAttribute('data-toc');
        const titleEl = s.querySelector('h2') || s.querySelector('h3');
        let title = titleAttr || (titleEl ? titleEl.innerText : s.id);
        title = title.replace(/^[0-9]+\s*/, '');
          ...
          <Center>
            ...
            >
              <iframe
                ...
                src="/static-blogs/bodhan-tts/indic-speak.html#cover"
                style={{
                  width: "100%",
                  height: "100%",
                  border: "none",
        ...
        <Flex justify="center" w="100%" px={{ base: 2, md: 4 }}>
          ...
          >
            ...
            <iframe
              ...
              src="/static-blogs/bodhan-tts/indic-speak.html"
              style={{
                width: "100%",
                minHeight: "80vh",
                border: "none",
