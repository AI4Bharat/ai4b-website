...
// Each stage is now one <g> so it animates as a unit, so the tags sit a level down.
const flDeep = (tag) => fl.children.reduce((a, c) =>
  a.concat(c.tagName === tag ? [c] : (c.children || []).filter((k) => k.tagName === tag)), []);
t('flow has six stages', flDeep('RECT').length, 6);
t('flow stages are grouped for the draw-in',
  fl.children.filter((c) => c.className.indexOf('fl-card') >= 0).length, 6);
t('every wire is drawable', fl.children
  .filter((c) => c.tagName === 'PATH' && c.className.indexOf('fl-wire') >= 0)
  .every((c) => c.attrs.pathLength === '1' && /animation-delay/.test(c.attrs.style || '')), true);
t('flow has five connectors', fl.children.filter((c) => c.tagName === 'PATH' && c.attrs.d && /^M[\d.]+ [\d.]+[HV]/.test(c.attrs.d)).length, 5);
t('flow names Vocos last', flDeep('TEXT').map((e) => e.textContent).includes('Vocos'), true);
